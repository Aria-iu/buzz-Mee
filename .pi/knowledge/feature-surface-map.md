...
|---|---|---|---|---|
| Community/onboarding | `desktop/src/features/communities/`、`onboarding/` | `features/invites/`、`settings/` | `buzz-relay/src/tenant.rs`、`buzz-db/src/community.rs`、invite APIs | channel/community相关命令 |
| Stream/messages | `features/messages/`、`chat/`、`channels/` | `features/channels/` | `handlers/ingest.rs`、`req.rs`、`buzz-db/src/event.rs`、`thread.rs` | `buzz messages` |
| Forum | `features/forum/` | `features/forum/` | forum kinds、ingest、thread store | `buzz messages --kind`、vote |
| DMs | message/channel UI | channels/DM presentation | `buzz-db/src/dm.rs`、DM command handlers | `buzz dms` |
| Home/activity | `features/home/`、`notifications/`、`reminders/` | `features/home/`、`activity/` | `buzz-db/src/feed.rs`、reminder kinds | `buzz feed` |
...
| Workflows | `features/workflows/` | 暂无同等完整编辑器 | `buzz-workflow`、`workflow_sink.rs`、`buzz-db/src/workflow.rs` | `buzz workflows` |
...
| Projects/repos | `features/projects/` | 当前非主要 surface | `api/git/`、`buzz-db/src/git_repo.rs`、NIP-34/MP kinds | `buzz repos/projects/patches/issues/pr` |
